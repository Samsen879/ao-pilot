...
func checkStore(dataDir string) doctorCheck {
	...
	case errors.Is(err, fs.ErrNotExist):
		return doctorCheck{
			...
			Message: "database not created yet; run `ao-pilot start` to initialize and migrate it",
		}
	default:
...
func (c *commandContext) checkGit(ctx context.Context) doctorCheck {
	...
	if cmp < 0 {
		return doctorCheck{Level: doctorFail, Section: doctorSectionTools, Name: "git", Message: fmt.Sprintf("%s (version %s; AO requires >= %s for worktrees)", path, version, minGitVersion)}
	}
