...
function buildTaskGraphBlockedStateReport({
  snapshot,
  projectId,
  resolvedRepoRoot,
  auditEntries,
  ...
  const taskGraph = snapshot.task_graph_inspection;
  const records = (key) => (Array.isArray(state?.[key]) ? state[key] : []);
  const controllerRuntime = buildControllerRuntimeSummary({ snapshot, projectId, now });
  const repoKnowledgeRecord = records('repo_knowledge').find(
  ...
  return {
    ...
    state,
    summary: {
      managed_task_count: records('managed_tasks').length,
      task_graph_available: true,
      task_graph_healthy: false,
      task_graph_finding_count: taskGraph.findings.length,
      pr_binding_count: records('pr_bindings').length,
      active_ownership_lease_count: records('ownership_leases')
        .filter((lease) => lease.status === 'active').length,
      active_controller_lease_count: null,
      action_count: records('actions').length,
      active_override_count: records('overrides')
        .filter((override) => override.status === 'active').length,
      controller_mode_count: records('controller_modes').length,
      controller_modes: summarizeControllerModes(records('controller_modes')),
      controller_health: controllerRuntime.controllerHealth,
      observation_count: records('observations').length,
      controller_cursor_count: records('controller_cursors').length,
      checkpoint_count: records('checkpoints').length,
      valid_checkpoint_count: 0,
      stale_checkpoint_count: 0,
      invalid_checkpoint_count: records('checkpoints').length,
      handoff_request_count: records('handoff_requests').length,
      handoff_claim_count: records('handoff_claims').length,
      handoff_decision_count: records('handoff_decisions').length,
      handoff_transfer_count: records('handoff_transfers').length,
      active_handoff_count: records('handoff_requests')
        .filter((request) => ['open', 'accepted'].includes(request.status)).length,
      controller_run_metric_count: records('controller_run_metrics').length,
      execution_attempt_metric_count: records('execution_attempt_metrics').length,
      repo_knowledge_count: records('repo_knowledge').length,
    ...
    continuity: { summary: summarizeContinuityReports([]), inspections: [] },
    reviews: { summary: summarizeReviewInspections([]), inspections: [] },
    tasks: {
      summary: {
    ...
    task_graph: snapshot.bootstrapped ? taskGraph : null,
    debt: {
      summary: {
        category_counts: {
          keep_evidence: 0,
    ...
    handoffs: { inspections: [] },
    repo_knowledge: { record: repoKnowledgeRecord, inspection: repoKnowledgeInspection },
    metrics: {
      summary: metricsReport.summary,
      recent_traces: metricsReport.recent_traces,
    },
    audit: { recent_entries: recentEntries },
  };
}
...
export async function loadAoStateReport({
  ...
  const snapshot = repository.getDiagnosticSnapshot();
  ...
  if (snapshot.task_graph_inspection?.structurally_healthy === false) {
    return buildTaskGraphBlockedStateReport({
      snapshot,
      projectId,
      resolvedRepoRoot,
      auditEntries,
  ...
  const taskGraph = snapshot.task_graph_inspection ?? inspectTaskGraph({
  ...
  return {
    ...
    summary: {
      ...
      task_graph_available: snapshot.bootstrapped,
      task_graph_healthy: snapshot.bootstrapped ? taskGraph.healthy : null,
      task_graph_finding_count: taskGraph.findings.length,
